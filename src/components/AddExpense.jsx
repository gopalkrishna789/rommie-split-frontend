...
export default function AddExpense({ members, onAdd, onClose }) {
  ...
  const percentTotal    = Object.values(percentShares).reduce((s, v) => s + (parseFloat(v) || 0), 0);
  ...
  const fillPercentRemaining = (memberId) => {
    const others      = members.filter((m) => m.id !== memberId);
    const othersTotal = others.reduce((s, m) => s + (parseFloat(percentShares[m.id]) || 0), 0);
    const remaining   = 100 - othersTotal;
    if (remaining >= 0) setPercentShares((p) => ({ ...p, [memberId]: Math.round(remaining) }));
  };
  ...
  const handleReceiptUpload = (e) => {
    ...
    reader.onload = (ev) => {
      setReceiptBase64(ev.target.result);
  ...
  const handleSubmit = async (e) => {
    ...
    if (splitMode === 'percentage' && Math.abs(percentRemaining) > 0.5)
      return setError(`Percentages must add up to 100%. Remaining: ${percentRemaining.toFixed(1)}%`);

    setLoading(true);
    try {
      // Convert percentage to custom paise shares for the API
      const resolvedCustomShares = splitMode === 'percentage'
        ? Object.fromEntries(members.map((m) => [m.id, Math.round(totalPaise * (parseFloat(percentShares[m.id]) || 0) / 100)]))
        : splitMode === 'custom'
      ...
      await onAdd({
        ...
        date,
        splitMode: splitMode === 'percentage' ? 'custom' : splitMode,
        customShares: resolvedCustomShares,
        isRecurring,
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl overflow-hidden max-h-[95vh] flex flex-col"
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1">
          <div className="p-5 space-y-5">
            ...
            <div>
              ...
              >
                {selectedCat ? (
                  <>
                    ...
                    <span className="flex-1 font-semibold text-sm" style={{ color: '#1C1C1E' }}>
                      {selectedCat.label}
                    </span>
                  </>
                ) : (
              ...
              {showCatPicker && (
                <div
                  className="mt-2 grid grid-cols-3 gap-2 p-3 rounded-2xl border animate-fade-in"
            ...
            <div>
              ...
              {receiptPreview ? (
                <div className="relative rounded-2xl overflow-hidden border-2" style={{ borderColor: '#6366F1' }}>
                  ...
                  <button
                    type="button"
                    onClick={() => { setReceiptBase64(null); setReceiptPreview(null); }}
                  ...
                  <div
                    className="absolute bottom-2 left-2 flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-semibold"
            ...
            <div
              ...
              style={{
                borderColor: isRecurring ? '#6366F1' : '#E5E5E3',
                background: isRecurring ? '#F7FFF9' : '#FFFFFF',
              }}
            >
              <button
                type="button"
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs mt-0.5" style={{ color: '#6B7280' }}>
                    Auto-creates this expense every month
                  </p>
                </div>
                <div
              ...
              <div className="grid grid-cols-2 gap-2">
                {members.map((member) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold truncate" style={{ color: '#1C1C1E' }}>
                      {member.name}
                    </span>
                    {payerId === member.id && (
                      <Check size={14} className="ml-auto flex-shrink-0" style={{ color: '#6366F1' }} />
                    )}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              {splitMode === 'percentage' && (
                <div
                  className="space-y-2 rounded-2xl p-3 border"
                  style={{ background: '#F7F7F5', borderColor: '#E5E5E3' }}
                >
                  <div className="flex justify-between text-xs px-1 mb-1" style={{ color: '#6B7280' }}>
                    ...
                    <span>
                      ...
                      <span
                        className="font-bold"
                        style={{
                          color: percentRemaining < 0 ? '#FF6B35' : percentRemaining === 0 ? '#6366F1' : '#F7C948',
                        }}
                      >
                        {percentRemaining.toFixed(1)}%
                        {percentRemaining < 0 ? ' over' : percentRemaining > 0 ? ' left' : ' ✓'}
                      </span>
                    </span>
                  </div>
                  {members.map((member) => {
                    const pct = parseFloat(percentShares[member.id]) || 0;
                    const share = Math.round(totalPaise * pct / 100);
                    return (
                      <div
                        key={member.id}
                      ...
                      >
                        <MemberAvatar member={member} size="sm" />
                        <span className="text-sm font-medium flex-1 truncate" style={{ color: '#1C1C1E' }}>
                          {member.name}
                        </span>
                        {totalPaise > 0 && (
                        ...
                        <div className="flex items-center gap-1">
                          <input
                            type="number"
                            value={percentShares[member.id]}
                            onChange={(e) => setPercentShares((p) => ({ ...p, [member.id]: e.target.value }))}
                            placeholder="0"
                            min="0"
                            max="100"
                            step="1"
                            className="w-16 border rounded-lg px-2 py-1.5 text-sm text-right focus:outline-none input-focus"
                            style={{ borderColor: '#E5E5E3' }}
                          />
                        ...
                        <button
                          type="button"
                          onClick={() => fillPercentRemaining(member.id)}
                  ...
                  {percentTotal > 0 && (
                    <div className="flex h-2 rounded-full overflow-hidden gap-0.5 mt-2">
                      {members.map((m, i) => {
                        const pct = parseFloat(percentShares[m.id]) || 0;
                        const colors = ['#6366F1','#f97316','#06b6d4','#a855f7','#F7C948','#ec4899'];
                        return pct > 0 ? (
                          <div key={m.id} className="h-full rounded-full"
                            style={{ width: `${(pct / Math.max(percentTotal, 100)) * 100}%`, background: m.color || colors[i % colors.length] }} />
                        ) : null;
                      })}
                    </div>
                  )}
            ...
            {selectedCat && totalPaise > 0 && (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold truncate" style={{ color: '#4F46E5' }}>
                    {purpose || selectedCat.label}
                  </p>
